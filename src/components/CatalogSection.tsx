...
export const CatalogSection: React.FC<CatalogSectionProps> = ({ currentLang }) => {
  ...
  return (
    <section id="catalogo" className="py-16 sm:py-24 bg-slate-50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filter Controls Bar */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10 glass-card p-3 sm:p-4 rounded-2xl border border-slate-200">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${
                  activeCategory === cat.id
                    ? 'bg-[#E3004F] text-zinc-900 shadow-[0_0_15px_rgba(227,0,79,0.5)]'
                    : 'bg-slate-100/80 text-slate-600 hover:text-zinc-900 hover:bg-slate-200'
                }`}
              >
                {cat.label}
              </button>
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-slate-100/90 border border-slate-200 focus:border-[#E3004F] text-slate-800 text-xs sm:text-sm rounded-xl pl-10 pr-4 py-2.5 outline-none transition"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-zinc-900 text-xs"
              >
                ✕
              </button>
            )}
        ...
        {filteredVarieties.length === 0 ? (
          <div className="text-center py-16 glass-card rounded-3xl border border-slate-200">
            ...
            <button
              ...
              className="mt-4 text-xs bg-[#E3004F] text-zinc-900 px-4 py-2 rounded-full font-medium"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredVarieties.map((rose, idx) => (
              <motion.div
                ...
                className="glass-card rounded-2xl overflow-hidden border border-slate-200/80 hover:border-rose-500/40 transition-all duration-300 group flex flex-col justify-between"
              >
                ...
                <div className="relative h-64 overflow-hidden bg-slate-50">
                  ...
                  {/* Category Pill Badge */}
                  <span className="absolute top-3 left-3 bg-white/80 backdrop-blur-md border border-rose-500/30 text-rose-300 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                    {rose.colorName}
                  </span>
                  ...
                  <button
                    ...
                    className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/70 hover:bg-[#E3004F] text-zinc-900 flex items-center justify-center border border-black/20 transition-all opacity-0 group-hover:opacity-100 transform translate-y-1 group-hover:translate-y-0"
                  ...
                  <div className="absolute bottom-3 left-3 right-3">
                    <h3 className="text-2xl font-display font-bold text-zinc-900 group-hover:text-rose-300 transition-colors">
                      {rose.name}
                    </h3>
                  </div>
                </div>
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-100/60 p-2.5 rounded-xl border border-slate-200 mb-3">
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {rose.stemLengthsCm.slice(0, 5).map((len) => (
                        <span
                          key={len}
                          className="bg-slate-50 text-slate-700 border border-slate-200 text-[10px] font-mono px-2 py-0.5 rounded"
                        >
                          {len}cm
                        </span>
                      ))}
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-200/80">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full inline-flex items-center justify-center gap-2 bg-[#E3004F] hover:bg-[#ff1a66] text-zinc-900 text-xs font-bold py-2.5 px-4 rounded-xl shadow-[0_0_15px_rgba(227,0,79,0.3)] transition"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-center text-[11px] text-slate-600 hover:text-zinc-900 py-1 font-medium transition"
                    >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-md">
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-4 right-4 z-20 w-9 h-9 rounded-full bg-white/70 hover:bg-[#E3004F] text-zinc-900 flex items-center justify-center border border-black/20 transition"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2">
                  ...
                  <div className="p-6 sm:p-8 space-y-5">
                    <div>
                      ...
                      <h3 className="text-3xl font-display font-bold text-zinc-900 mt-1">
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-600">Calibre del Botón:</span>
                        <span className="text-zinc-900 font-bold">{selectedVariety.headSizeCm}</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-600">Vida Promedio en Florero:</span>
                        <span className="text-zinc-900 font-bold">{selectedVariety.vaseLifeDays} Días</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-600">Forma de Apertura:</span>
                        <span className="text-zinc-900 font-bold">{selectedVariety.bloomType}</span>
                      </div>
                    </div>
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition ${
                              selectedStemLength === len
                                ? 'bg-[#E3004F] text-zinc-900 shadow-[0_0_10px_rgba(227,0,79,0.5)]'
                                : 'bg-slate-100 text-slate-600 border border-slate-200 hover:text-zinc-900'
                            }`}
                          >
                            {len} cm
                          </button>
                    ...
                    <div className="flex items-center justify-between bg-slate-100/80 p-3 rounded-xl border border-slate-200">
                      <span className="text-xs text-slate-700 font-medium">Cantidad Estimada (Bonches):</span>
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => setBunchQty(Math.max(10, bunchQty - 10))}
                          className="w-7 h-7 rounded-lg bg-slate-200 hover:bg-slate-700 text-zinc-900 font-bold text-sm"
                        >
                          -
                        </button>
                        <span className="text-sm font-bold text-zinc-900 font-mono">{bunchQty}</span>
                        <button
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full inline-flex items-center justify-center gap-2 bg-[#E3004F] hover:bg-[#ff1a66] text-zinc-900 font-bold text-sm py-3 px-6 rounded-2xl shadow-[0_0_20px_rgba(227,0,79,0.5)] transition"
                    >
