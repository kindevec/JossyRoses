...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      ...
      <div className="bg-[#0b0f17] border-b border-rose-500/10 text-xs text-slate-300 py-1.5 px-4 hidden md:block relative z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="flex items-center gap-1 text-slate-300 hover:text-white px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-xs font-semibold"
            >
      ...
      <header
        className={`sticky top-0 z-40 w-full transition-all duration-300 ${
          isScrolled
            ? 'bg-[#000000]/90 backdrop-blur-md border-b border-rose-500/20 shadow-2xl py-2.5 sm:py-3'
            : 'bg-gradient-to-b from-black/95 to-transparent py-3 sm:py-4'
        }`}
      >
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-2">
          ...
          <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-slate-200 hover:text-[#E3004F] transition-colors relative py-1 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-[#E3004F] hover:after:w-full after:transition-all after:duration-300 whitespace-nowrap"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0">
            ...
            <button
              onClick={onToggleLang}
              className="lg:hidden flex items-center gap-1 text-slate-200 px-2 py-1.5 rounded-full bg-slate-900/90 border border-slate-800 text-[11px] sm:text-xs font-semibold shrink-0 hover:border-rose-500/50 transition"
