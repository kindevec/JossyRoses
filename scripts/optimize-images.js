...
const TARGET_DIRS = ['public/images', 'public', 'src/assets'];
...
async function optimizeImages() {
  ...
  for (const relDir of TARGET_DIRS) {
    ...
    for (const file of files) {
      ...
      console.log(
        `| ${relName.padEnd(30)} | ${formatSize(originalSize).padEnd(10)} | ${formatSize(webpSize).padEnd(10)} | ${formatSize(avifSize).padEnd(10)} | ${(webpSavings + '%').padEnd(11)} | ${(avifSavings + '%').padEnd(11)} |`
      );
    }
  }
